...
class TD_VAE(nn.Module):
    ...
    def __init__(
        ...
        self.l2_b_to_z = DBlock(b_size, d_block_hidden_size, z_size)  # layer 2
        self.l1_b_to_z = DBlock(b_size + z_size, d_block_hidden_size, z_size)  # layer 1

        ## Given belief and state at time t2, infer the state at time t1
        ## infer state
        self.l2_infer_z = DBlock(
            b_size + 2 * z_size, d_block_hidden_size, z_size
        )  # layer 2
        self.l1_infer_z = DBlock(
            b_size + 2 * z_size + z_size, d_block_hidden_size, z_size
        )  # layer 1

        ## Given the state at time t1, model state at time t2 through state transition
        ## state transition
        self.l2_transition_z = DBlock(2 * z_size, d_block_hidden_size, z_size)
        self.l1_transition_z = DBlock(2 * z_size + z_size, d_block_hidden_size, z_size)

        ## state to observation
        self.z_to_x = Decoder(2 * z_size, decoder_hidden_size, x_size)
    ...
    def calculate_loss(
        ...
        loss = (
            0.5
            * torch.sum(
                ((t1_l2_pb_z_mu - t1_l2_qs_z) / torch.exp(t1_l2_pb_z_logsigma)) ** 2, -1
            )
        ...
        loss += (
            0.5
            * torch.sum(
                ((t1_l1_pb_z_mu - t1_l1_qs_z) / torch.exp(t1_l1_pb_z_logsigma)) ** 2, -1
            )
        ...
        loss += torch.sum(
            -0.5 * t2_l2_z_epsilon**2
            - 0.5 * t2_l2_z_epsilon.new_tensor(2 * np.pi)
            - t2_l2_z_logsigma,
            dim=-1,
        )
        loss += torch.sum(
            -0.5 * t2_l1_z_epsilon**2
        ...
        loss += torch.sum(
            0.5 * ((t2_l2_z - t2_l2_t_z_mu) / torch.exp(t2_l2_t_z_logsigma)) ** 2
            + 0.5 * t2_l2_z.new_tensor(2 * np.pi)
            + t2_l2_t_z_logsigma,
            -1,
        )
        loss += torch.sum(
            0.5 * ((t2_l1_z - t2_l1_t_z_mu) / torch.exp(t2_l1_t_z_logsigma)) ** 2
            + 0.5 * t2_l1_z.new_tensor(2 * np.pi)
            + t2_l1_t_z_logsigma,
            -1,
        )
        ...
        loss += -torch.sum(
            self.x[:, t2, :] * torch.log(t2_x_prob)
            + (1 - self.x[:, t2, :]) * torch.log(1 - t2_x_prob),
            -1,
        )
    ...
    def rollout(
        ...
        """
        Rollout the model for jumpy prediction from time t1 to time t2

        Parameters
        ----------
        images : torch.Tensor
            The input tensor of images
        t1 : int
            The time point t1
        t2 : int
            The time point t2 to start the rollout prediction

        Returns
        -------
        torch.Tensor
            The predicted tensor of images
        """
        self.forward(images)
